...
export default function AddProgramPage() {
  ...
  const validateForm = () => {
    ...
    if (formData.noOfParticipation <= 0) {
      newErrors.noOfParticipation = "No of participation must be greater than 0"
    }
    if (formData.candidatesPerParticipation <= 0) {
      newErrors.candidatesPerParticipation = "Candidates per participation must be greater than 0"
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const programData = {
        ...formData,
        status: formData.endingTime && formData.date && formData.startingTime && formData.venue ? "Scheduled" : "Draft",
      }
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="noOfParticipation">No. of Participation *</Label>
                  <Input
                    id="noOfParticipation"
                    type="number"
                    min="1"
                    value={formData.noOfParticipation}
                    onChange={(e) => {
                      const value = Number.parseInt(e.target.value) || 0
                      setFormData({ ...formData, noOfParticipation: value })
                    }}
                    className={errors.noOfParticipation ? "border-red-500" : ""}
                  />
                  {errors.noOfParticipation && <p className="text-sm text-red-500">{errors.noOfParticipation}</p>}
                </div>

                <div className="space-y-2">
                  <Label htmlFor="candidatesPerParticipation">Candidates per Participation *</Label>
                  <Input
                    id="candidatesPerParticipation"
                    type="number"
                    min="1"
                    value={formData.candidatesPerParticipation}
                    onChange={(e) => {
                      const value = Number.parseInt(e.target.value) || 0
                      setFormData({ ...formData, candidatesPerParticipation: value })
                    }}
